...
class HomeController extends BaseController {
  ...
  void shareUrl() {
    if (checkRequestResponseModel.value.data.isNotEmpty) {
      ...
      String data =
          "ETO RIDE : ${datum.user?.firstName ?? ""} ${datum.user?.lastName ?? ""} is riding in ${datum.serviceType?.name ?? ""} would like to share his ride ${ApiUrl.BASE_URL}/track/${datum.id ?? "0"}";

      FlutterShare.share(title: "ETO Ride Driver Share Ride", text: data);
    }
  }
